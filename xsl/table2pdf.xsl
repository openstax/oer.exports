...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="style.param.determiner">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$style.name='rowsep' and
          (parent::db:row[not(following-sibling::db:row)]
           and not(ancestor::*[2][preceding-sibling::db:tfoot or self::db:thead]
      or (ancestor::db:tgroup[following-sibling::db:tgroup] and not(ancestor::db:entrytbl))) or
          @morerows=count(parent::db:row/following-sibling::db:row)
           and not(ancestor::*[2][preceding-sibling::db:tfoot or self::db:thead]
      or (ancestor::db:tgroup[following-sibling::db:tgroup] and not(ancestor::db:entrytbl))))">
        <xsl:call-template name="style.maker">
      ...
      <xsl:when test="$style.name='colsep' and
          not(following-sibling::*) and
          not(parent::db:row/preceding-sibling::db:row/db:entry[position()=last()][(@morerows + count(parent::db:row/preceding-sibling::db:row)) &gt;= count(current()/parent::db:row/preceding-sibling::db:row)])">
        <xsl:call-template name="style.maker">
          <xsl:with-param name="style.param" select="'0'"/>
          <xsl:with-param name="style.name" select="'colsep'"/>
        </xsl:call-template>
  ...
  <xsl:template name="style.maker">
    ...
    <xsl:choose>
      <xsl:when test="$style.name='colsep'">
        <xsl:choose>
          <xsl:when test="$style.param='0'">
            <xsl:text>colsep-off </xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>colsep-on </xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:when test="$style.name='rowsep'">
        <xsl:choose>
          <xsl:when test="$style.param='0'">
            <xsl:text>rowsep-off </xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>rowsep-on </xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:when test="$style.name='align'">
