...
class Test_collectiondbk2pdf(unittest.TestCase):
    ...
    def setUpClass(cls):
        if not exists(join(INPUT_DIR,COLLECTION)):
            if exists(join(INPUT_DIR,COLLECTION+".zip")):
                command=["unzip", join(INPUT_DIR,COLLECTION+".zip"),"-d",INPUT_DIR]
        ...
        print("---setUpClass: script=collectiondbk2pdf input={1} runtime={0} seconds ---".format(cls.setup_runtime,COLLECTION))
    ...
    def test_pdf_gen_accuracy(self):
        ...
        expected_files_list = [ join(OUTPUT_DIR,f) for f in listdir(OUTPUT_DIR) if isfile(join(OUTPUT_DIR,f)) and "png" in f and "expected" in f ]
        actual_files_list = [ join(OUTPUT_DIR,f) for f in listdir(OUTPUT_DIR) if isfile(join(OUTPUT_DIR,f)) and "png" in f and "test" in f]
